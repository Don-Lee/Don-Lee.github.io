...
Handler 引起的内存泄漏   
静态集合类引起内存泄露(集合对象没有及时清理引起的内存泄漏。通常会把一些对象装入到集合中，当不使用的时候一定要记得及时清理集合，让相关对象不再被引用)    
单例模式引起的内存泄漏    
解决方案：   
1)、尽量在不显示的时候释放内存                       
2)、对不需要强引用的对象采用弱引用                                   
...
14)、尽量用静态内部类   
15)、减少内存对象的占用(使用ArrayMap/SparseArray代替hashmap; 对质量要求不大的图片采用 RBG_565 格式;对大的图片进行压缩)
